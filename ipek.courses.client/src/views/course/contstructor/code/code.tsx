...
const testLang = store.languages.getLangs.find((l) => l.id === 'typescript') as ILanguage;
const ConstructorCode = ({ isReadonly, theory }: IConstructorCode): React.JSX.Element => {
	...
	const calcMaxHeightEditor = (): void => {
		if (boxQuestions.current !== null && boxQuestion.current !== null) {
			const h =
				boxQuestions.current?.clientHeight - boxQuestion.current?.clientHeight - 16;
	...
	return (
		<div className='constructor-code' ref={boxConstructor}>
			...
			<DragLine
				boxLeftRef={boxTheory}
				boxRightRef={boxQuestions}
				boxParentRef={boxConstructor}
			/>
